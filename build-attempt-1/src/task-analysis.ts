...
const STOP_WORDS = new Set([
  ...
  "in", "into", "is", "it", "make", "of", "on", "or", "that", "the", "this",
  "to", "without", "with",
]);
...
const GENERIC_PATH_TERMS = new Set(["docs", "fixtures", "lib", "sample", "src", "test", "tests"]);
...
const detectTaskFamily = (description: string): TaskAnalysis["taskFamily"] => {
...
const scoreFile = (
  ...
  for (const explicitPath of explicitPaths) {
    if (path !== explicitPath && !path.endsWith(`/${explicitPath}`)) continue;
    score += 20;
    reasons.push("explicitly named in task");
  }
  for (const keyword of keywords) {
    if (!pathTerms.has(keyword)) continue;
  ...
  if (taskFamily === "test" || taskFamily === "bug-fix") {
    if (file.category === "test") {
      score += 1.5;
      reasons.push("task likely requires test evidence");
    }
  }
  if (taskFamily === "documentation" && file.category === "documentation") {
    score += 2;
...
export const analyzeTask = (
  ...
  ].join(" ").match(/[a-zA-Z0-9_-]+(?:\/[a-zA-Z0-9_.-]+)+/gu)
    ?.map((path) => path.replace(/[.,;:)]+$/u, "").toLowerCase()) ?? [];
  const openEndedSignalCount = OPEN_ENDED_PATTERNS.filter((pattern) => pattern.test(task.description)).length;

  const scoredFiles = manifest.files
    .filter(({ category }) => category !== "binary" && category !== "generated")
    .map((file) => ({ file, ...scoreFile(file, keywords, explicitPaths, taskFamily) }))
    .filter(({ score }) => score > 0)
    .sort((left, right) => right.score - left.score || left.file.path.localeCompare(right.file.path));

  const fallbackFiles = manifest.files
    .filter(({ category }) => ["source", "test", "manifest"].includes(category))
    .sort((left, right) => left.approximateTokens - right.approximateTokens)
    .slice(0, 8)
    .map((file) => ({
      file,
      score: 0.25,
  ...
  const boundednessScore = Math.max(
    0,
    Math.min(1, 0.65 + task.prohibitedChanges.length * 0.08 - openEndedSignalCount * 0.3),
  );
  const verifiabilityScore = Math.min(
    1,
    (task.verifierCommand ? 0.55 : 0)
  ...
  if (openEndedSignalCount > 0) signals.push("request contains open-ended language");
  if (!manifest.baselineSignals.hasTests) signals.push("repository scan found no tests");
  if (!task.verifierCommand) signals.push("no explicit verifier command supplied");
  if (manifest.oversizedFiles.length > 0) signals.push(`${manifest.oversizedFiles.length} oversized text files`);
  ...
  return taskAnalysisSchema.parse({
    ...
    boundednessScore,
    verifiabilityScore,
    likelyRelevantFiles: selectedFiles.map(({ file, score, reasons }) => ({
      path: file.path,
      score,
      reasons,
